...
function mapTest() {
	d3.csv('../data/brewery_count_by_state_1984_2017.csv', function(err, data) {
		...
		let option_select = d3.select('#selectors').append('select').attr('class', 'option-select');

		for (let i = 0; i < fields.length; i++) {
			if (fields[i] !== config.state) {
				let opt = option_select.append('option').attr('value', fields[i]).text(fields[i]);
		...
		let quantize = d3.scale.quantize().domain([ 0, 1.0 ]).range(
		...
		let svg = d3.select('#canvas-svg').append('svg').attr('width', width).attr('height', height);
				...
				function drawMap(dataColumn) {
					...
					svg
						...
						.on('mousemove', function(d) {
							...
							html +=
								'<div><span> brewery count: ' +
								dataMap[id_name_map[d.id]][selectedYear] +
								'</span></div>';
